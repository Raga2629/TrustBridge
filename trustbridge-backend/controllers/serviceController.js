...
const createService = async (req, res) => {
  try {
    ...
    // Gate: Provider must have verified phone AND completed identity verification
    const User = require('../models/User');
    const IdentityVerification = require('../models/IdentityVerification');

    const user = await User.findById(req.user._id).select('isPhoneVerified');
    const identityRecord = await IdentityVerification.findOne({ userId: req.user._id });

    if (!user || !user.isPhoneVerified) {
      return res.status(403).json({
        message: 'Phone number verification required before posting services.',
        action: 'verify_phone',
        redirectTo: '/provider/verify'
      });
    }

    // Block only if identity was explicitly rejected (not just pending or not submitted)
    if (identityRecord && identityRecord.verificationStatus === 'rejected' && identityRecord.matchScore < 50) {
      return res.status(403).json({
        message: 'Identity verification failed. Please re-upload your Aadhaar and selfie.',
        action: 'verify_identity',
        redirectTo: '/provider/verify'
      });
    }

    // If no identity record at all, require it
    if (!identityRecord) {
      return res.status(403).json({
        message: 'Identity verification required before posting services.',
        action: 'verify_identity',
        redirectTo: '/provider/verify'
      });
    }
    ...
    const { verifyServiceImage } = require('../utils/imageVerificationService');
    const imageVerification = await verifyServiceImage(
      path.resolve(req.files.serviceImage[0].path),
      name,
      category,
      description
    );

    // AI decides: image matches category → approved, doesn't match → rejected
    let approvalStatus, verified, imageVerificationNote;

    if (imageVerification.isValid) {
      approvalStatus = 'approved';
      verified = true;
      imageVerificationNote = `✅ AI verified: ${imageVerification.detectedContent} matches ${category} (${imageVerification.confidence}% confidence)`;
    } else {
      // Block submission — don't even save the service
      return res.status(400).json({
        message: `Service image rejected: ${imageVerification.reason}`,
        detectedContent: imageVerification.detectedContent,
        action: 'upload_correct_image'
      });
    }
    ...
    const serviceData = {
      ...
      imageVerificationScore: imageVerification.confidence
    };
    ...
    res.status(201).json({
      ...
      message: approvalStatus === 'approved'
        ? '✅ Service added and approved! It is now live on TrustBridge.'
        : '❌ Service rejected by AI verification.'
    });
  } catch (error) {
...
const getAllServices = async (req, res) => {
  try {
    ...
    let query = { approvalStatus: 'approved' }; // Only show admin-approved services publicly
...
const rejectService = async (req, res) => {
  try {
    ...
    service.approvalNote = req.body.reason || 'Rejected by admin';
    await service.save();
...
// @desc    Get provider's own services (including pending)
