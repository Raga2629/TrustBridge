...
const AdminDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [statsRes, usersRes, complaintsRes, reviewsRes, pendingServicesRes] = await Promise.all([
        ...
        axios.get('/services/admin/pending', config).catch(() => ({ data: [] }))
      ]);
    ...
    <div className="admin-dashboard">
      <div className="dashboard-container">
        ...
        {activeTab === 'pending-services' && (
          <div className="tab-content">
            <h3 style={{marginBottom:'16px'}}>Services Awaiting Approval</h3>
            {pendingServices.length === 0 ? (
              <div className="empty-state"><p>✅ No pending services</p></div>
            ) : (
              <div className="complaints-grid">
                {pendingServices.map(service => (
                  <div key={service._id} className="complaint-card" style={{borderLeft:'4px solid #f57c00'}}>
                    <div className="complaint-header">
                      <h4>{service.name}</h4>
                      <span className="status-badge" style={{background:'#f57c00',color:'white'}}>PENDING</span>
                    </div>
                    <div className="complaint-details">
                      <p><strong>Provider:</strong> {service.provider?.name || 'Unknown'}</p>
                      <p><strong>Email:</strong> {service.provider?.email}</p>
                      <p><strong>Category:</strong> {service.category}</p>
                      <p><strong>Area:</strong> {service.area}, {service.city}</p>
                      <p><strong>Description:</strong> {service.description?.slice(0,100)}...</p>
                      <p><strong>Phone:</strong> {service.contactPhone}</p>
                      {service.serviceImageUrl && (
                        <p><strong>Image:</strong> <a href={`http://localhost:5000/${service.serviceImageUrl}`} target="_blank" rel="noreferrer">View Image</a></p>
                      )}
                      {service.businessProofUrl && (
                        <p><strong>Business Proof:</strong> <a href={`http://localhost:5000/${service.businessProofUrl}`} target="_blank" rel="noreferrer">View Document</a></p>
                      )}
                      {service.imageVerificationNote && (
                        <p style={{
                          background: service.imageVerificationNote.startsWith('✅') ? '#e8f5e9' : '#fff8e1',
                          padding: '6px 10px',
                          borderRadius: '4px',
                          fontSize: '13px',
                          marginTop: '6px'
                        }}>
                          <strong>AI Image Check:</strong> {service.imageVerificationNote}
                        </p>
                      )}
                      <p><strong>Submitted:</strong> {new Date(service.createdAt).toLocaleDateString()}</p>
                    </div>
                    <div style={{display:'flex',gap:'8px',marginTop:'12px'}}>
                      <button
                        onClick={async () => {
                          try {
                            await axios.put(`/services/${service._id}/verify`, {}, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
                      ...
                      <button
                        onClick={async () => {
                          ...
                          if (!reason) return;
                          try {
                            await axios.put(`/services/${service._id}/reject`, { reason }, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
