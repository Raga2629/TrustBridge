...
const AddService = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const msg = response.data.message || 'Service added successfully!';
      alert(msg);
      navigate('/provider/dashboard');
    } catch (err) {
      console.error('Add service error:', err);
      const errData = err.response?.data;
      let errorMsg = errData?.message || 'Failed to add service. Please try again.';
      // Show detected content if image was rejected
      if (errData?.detectedContent) {
        errorMsg += ` (AI detected: "${errData.detectedContent}" in your image)`;
      }
      setError(errorMsg);
    } finally {
      setLoading(false);
  ...
  return (
    <div className="form-container">
      <div className="form-card">
        ...
        {error && (
          <div className="error-message" style={{background:'#fff3cd', border:'1px solid #ffc107', color:'#856404', padding:'12px 16px', borderRadius:'8px', marginBottom:'16px', fontSize:'14px', lineHeight:'1.5'}}>
            ⚠️ {error}
          </div>
        )}
